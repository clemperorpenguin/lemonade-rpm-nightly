Name:           lemonade
Version:        11.9.0
Release:        0.20260902git1e7ac6bc6%{?dist}
...
# The branch is a long-lived fork that carries the new front end but lags the
# released version, so Patch0 merges the newest upstream release back into it.
# Version is the project version of that merged tree -- i.e. the release's -- and
# Release encodes the snapshot date and branch commit (0.<date>git<sha>) so
# nightlies sort correctly among themselves.
# The tarball is generated by tito's SubmoduleAwareBuilder and submitted to COPR as an SRPM.
Source0:        %{name}-%{version}.tar.gz
# Brings the tracked branch up to the newest upstream release; regenerated each
# night by scripts/merge-release.sh. Do not edit by hand.
Patch0:         patches/0100-catch-up-to-release.patch
...
pushd %{upstream}
# Applied with git rather than %%patch: the catch-up patch includes a binary hunk
# for a test fixture the release adds, and patch(1) cannot apply git binary diffs.
git apply -p1 --binary %{PATCH0}
popd
%build
...
# Upstream installs its own metainfo for the web and Tauri front-ends. This spec
# ships hand-written metadata below and renames the web launcher's desktop entry,
# so the upstream files are both redundant and stale here.
# Both spellings are matched: the reverse-DNS id was ai.lemonade_server.* before
# 11.8 and ai.lemonadeserver.* after, and the branch and the release disagreed on
# it until the merge. Leaving either behind would fail the build on unpackaged
# files; the globs keep this a no-op if upstream drops or renames them again.
rm -f %{buildroot}%{_datadir}/metainfo/ai.lemonade_server.*.metainfo.xml
rm -f %{buildroot}%{_datadir}/metainfo/ai.lemonadeserver.*.metainfo.xml
...
# 11.8 renamed that id to match the Tauri bundle identifier (the old spelling had
# an underscore). The tray still falls back to the old name, but the desktop
# entries below have to agree with whatever is installed here.
install -Dpm 0644 %{upstream}/src/app/assets/logo.svg \
    %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/ai.lemonadeserver.Lemonade.svg
...
desktop-file-install \
    ...
    --set-icon=ai.lemonadeserver.Lemonade \
...
Icon=ai.lemonadeserver.Lemonade
Exec=lemonade-tray
Terminal=false
Categories=Utility;
EOF
...
%changelog
* Wed Sep 02 2026 clemperorpenguin <clem@pendragon.systems> 11.9.0-0.20260902git1e7ac6bc6
- nightly: GUI3_merging @ 1e7ac6bc6 on v11.9.0 (clem@pendragon.systems)
